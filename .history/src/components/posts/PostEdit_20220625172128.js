...
const PostEdit = () => {
  ...
  console.log("🚀 ~ file: PostEdit.js ~ line 24 ~ PostEdit ~ param", param);
  const idName = param.get("id");
  console.log("🚀 ~ file: PostEdit.js ~ line 26 ~ PostEdit ~ idName", idName);

  const WatchStatus = watch("status");

  const dispatch = useDispatch();
  const dt = useSelector((state) => state.global.dataUpdate);
  console.log("🚀 ~ file: PostEdit.js ~ line 30 ~ PostEdit ~ dt", dt);
